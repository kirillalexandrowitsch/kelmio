...
- Root `README.md` описывает **текущие реализованные** возможности продукта
  бизнес-языком и **без номеров версий**; он не рекламирует ещё не построенный
  функционал.
- Единственным фактическим источником статуса возможностей остаётся
  [capability baseline](product-capability-baseline.md); version plans
  детализируют будущую работу.
- README обновляется вместе с изменением набора реализованных возможностей
  (обычно в финальном QA-коммите версии), чтобы не опережать фактическое
  состояние.
...
V5 завершена. Перед V6 выполнен behavior-preserving редизайн интерфейса по
