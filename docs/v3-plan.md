...
Ключевой принцип V3: не добавлять новые enterprise-фичи ради масштаба, а сделать текущий функционал безопасным, поддерживаемым и готовым к запуску на собственном сервере.
...
- explicit `DATABASE_URL` имеет приоритет, иначе PostgreSQL URL безопасно собирается из `POSTGRES_*` значений с URL-encoding password;
...
Post-V3 cleanup audit завершен 7 июня 2026 года:

- frontend tests переведены на единый Vitest/React Testing Library runner;
- `App.tsx` сокращен до малого entrypoint, feature state вынесен в typed controllers;
- крупные issue/sprint handlers разделены по concerns;
- добавлен отдельный manual/weekly Full QA workflow;
- повторно прошли два setup-db, verify, smoke, integration, Playwright, race/vet,
  dependency audit, production Compose/images и isolated production-stack QA.
