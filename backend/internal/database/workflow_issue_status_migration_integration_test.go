...
func TestWorkflowIssueStatusMigrationEnablesCustomStatuses(t *testing.T) {
	...
	if len(applied) != 4 || applied[0].Version != 13 || applied[1].Version != 14 || applied[2].Version != 15 || applied[3].Version != 16 {
		t.Fatalf("applied migrations = %#v, want versions 13 through 16", applied)
	}
